...
- Default to `female-book-narrator-locked-v1` with its user-approved soft-v2 mastering and the matching female intro. Preserve the locked reference audio, VoxCPM2 generation parameters, fixed seed, natural timing, and soft mastering chain unless the user explicitly requests a different voice treatment. Use the male pair only when explicitly requested.
