...
assert.match(socialPublisher, /选择视频标注/, "publisher must support the current WeChat Channels video-label control");
assert.match(socialPublisher, /含AI產生內容/, "publisher must support the current localized AI-generated-content label");
assert.match(socialPublisher, /mark-tag-select/, "publisher must verify the closed selected-label control");
assert.match(socialPublisher, /未打开视频号.*封面编辑弹窗，已阻止正式发布/, "supplied WeChat Channels cover must fail closed");
